 ...
 * ported from the dsh-web-ui family's skill-explorer implementation.
 */
...
/** Parsed frontmatter fields consumed by the skill studio. */
...
/** Rewrite the frontmatter text of a SKILL.md content string (pure, no IO). */
export function rewriteFrontmatter(content: string, field: string, value: boolean): string {
  const match = /^---\r?\n([\s\S]*?)\r?\n---([\s\S]*)$/.exec(content)
...
/**
 * Rewrite one boolean frontmatter field in a SKILL.md file (atomic tmp+rename).
...
export function setFrontmatterField(file: string, field: string, value: boolean): Frontmatter {
  ...
  const rewritten = rewriteFrontmatter(content, field, value)
